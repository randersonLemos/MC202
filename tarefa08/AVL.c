...
void _FaltaParaListaLegal(AVL * raiz, int *counter)
{
  if( raiz != NULL)
  {
  _FaltaParaListaLegal(raiz->esq, counter);
  _FaltaParaListaLegal(raiz->dir, counter);

  if(raiz->info > raiz->counter)
    *counter = *counter + raiz->counter;
  if(raiz->info < raiz->counter)
    *counter = *counter + (raiz->counter - raiz->info);
  }
}

int FaltaParaListaLegal(AVL * raiz)
{
  int counter = 0;
  _FaltaParaListaLegal(raiz, &counter);
  return(counter);
}
