...
Hash *CriaTabelaHash(int n)
{
  ...
  hash->valor = (unsigned long long *)calloc(hash->m,sizeof(unsigned long long));
...
char InsereChave(Hash *hash, unsigned long long chave)
{
  int pos0, pos, j=1;
...
char RemoveChave(Hash *hash, unsigned long long chave)
{
  int pos0, pos, j=1;
...
void ImprimeTabelaHash(Hash *hash)
{
  ...
  for (i=0; i < hash->m; i++)
    if ((hash->valor[i]!=0) && (hash->valor[i]!=-1))
      printf("pos %3d; valor %30lld", i, hash->valor[i]);
}
